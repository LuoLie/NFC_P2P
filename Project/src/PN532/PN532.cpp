...
uint8_t PN532::RFConfig(uint8_t data)
{
	pn532_packetbuffer[0] = PN532_COMMAND_RFCONFIGURATION;
...
uint8_t PN532::mifareclassic_AuthenticateBlock (uint8_t *uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t *keyData)
{
    ...
    pn532_packetbuffer[2] = (keyNumber == MIFARE_CMD_AUTH_B) ? MIFARE_CMD_AUTH_B : MIFARE_CMD_AUTH_A;
...
uint8_t PN532::mifareclassic_ReadDataBlock (uint8_t blockNumber, uint8_t *data)
{
    ...
    /* Read the response packet */
	uint8_t buf[30] = {0};
    HAL(readResponse)(buf, sizeof(buf));
...
uint8_t PN532::mifareclassic_WriteDataBlock (uint8_t blockNumber, uint8_t *data)
{
    ...
    /* Read the response packet */
	int8_t length = HAL(readResponse)(pn532_packetbuffer, sizeof(pn532_packetbuffer));
...
bool PN532::inDataExchange(uint8_t *send, uint8_t sendLength, uint8_t *response, uint8_t *responseLength)
{
    ...
    int16_t status = HAL(readResponse)(response, *responseLength, 3000);
    if (status < 0) {
		DMSG("status is not ok: %d\r\n", status);
        return false;
    }

    if ((response[0] & 0x3f) != 0) {
        DMSG("Status code indicates an error: %x\r\n", response[0]);
        return false;
    }
...
bool PN532::inListPassiveTarget()
{
    ...
    pn532_packetbuffer[2] = 0;	//106 kbps type A (ISO/IEC14443 Type A)
	...
	if(pn532_packetbuffer[5] == 4)
	{
		_uidLen = 4;
		_uid[0] = pn532_packetbuffer[6];
		_uid[1] = pn532_packetbuffer[7];
		_uid[2] = pn532_packetbuffer[8];
		_uid[3] = pn532_packetbuffer[9];
	}
	else if(pn532_packetbuffer[5] == 7)
	{
		_uidLen = 7;
		_uid[0] = pn532_packetbuffer[6];
		_uid[1] = pn532_packetbuffer[7];
		_uid[2] = pn532_packetbuffer[8];
		_uid[3] = pn532_packetbuffer[9];
		_uid[4] = pn532_packetbuffer[10];
		_uid[5] = pn532_packetbuffer[11];
		_uid[6] = pn532_packetbuffer[12];
	}
...
int16_t PN532::tgGetData(uint8_t *buf, uint8_t len)
{
    ...
    int16_t status = HAL(readResponse)(buf, len, 450);
    if (0 >= status) {
        return status;
    }
...
bool PN532::tgSetData(const uint8_t *header, uint8_t hlen, const uint8_t *body, uint8_t blen)
{
    ...
    if (0 > HAL(readResponse)(pn532_packetbuffer, sizeof(pn532_packetbuffer), 450)) {
        return false;
    }
...
int PN532::InAutoPoll(uint8_t pollNr, uint8_t period, uint8_t type, uint8_t *buf, uint8_t len)
{
	uint8_t cmd[18];
	
	cmd[0] = PN532_COMMAND_INAUTOPOLL;
	cmd[1] = pollNr;
	cmd[2] = period;
	cmd[3] = type;
	
	if (HAL(writeCommand)(cmd, 4)) {
        return 0;
    }
 ...
 * Peer to Peer
 */
int8_t PN532::InJumpForDEP(uint8_t *buf, uint8_t len, uint16_t timeout)
{
    const uint8_t command[] = {
        PN532_COMMAND_INJUMPFORDEP,
        0x00, //passive mode
		0x00, //106kbps baud rate
		0x00	//no next field
    };
	...
	if(status < 0)
	{
		DMSG("InJumpForDEP command error: %d", status);
		return status;
	}
    status = HAL(readResponse)(buf, len, timeout);
    if (status > 0) {
        return 1;
    } else if (PN532_TIMEOUT == status) {
        return 0;
